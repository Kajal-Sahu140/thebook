...
<main class="content">
    <div class="container-fluid p-0">
        ...
        <div class="mb-3 d-flex justify-content-end flex-wrap">
            <form action="{{ route('admin.brand') }}" method="GET" class="d-flex flex-grow-1 flex-sm-grow-0">
                <input type="text" name="search" class="form-control me-2" placeholder="Search by Name" value="{{ request()->input('search') }}">
                <button type="submit" class="btn btn-primary">Search</button>
            </form>
           <a href="{{route('admin.brand.create')}}" class="btn btn-info ms-sm-3 mt-2 mt-sm-0">
                    ...
                    <tbody>
                        ...
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td>{{ $user->name }}</td>
                                <td>
                                   <img src="{{ asset('storage/app/public/' . $user->image) }}" height="50px" />
                                </td>
                               <td>{{ Str::limit($user->description, 30) ?? '' }}</td>
                                ...
                                <span class="badge {{ $user->status === 'active' ? 'bg-success' : 'bg-danger' }}">
                                 ...
                                 <td>
                                    <a href="{{ route('admin.brand.edit', base64_encode($user->id)) }}" class="btn btn-warning btn-sm">Edit</a>
                                    <form action="{{ route('admin.brand.destory', base64_encode($user->id)) }}" method="POST" style="display:inline-block;" class="delete-form">
                                        @csrf
                                        @method('DELETE')
