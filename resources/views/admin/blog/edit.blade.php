                ...
                <form id="editBlogForm" action="{{ route('admin.blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title_ar" id="title_ar" class="form-control @error('title_ar') is-invalid @enderror" 
                               value="{{ old('title_ar', $blog->title_ar) }}" placeholder="Enter Blog Title (AR)">
                        @error('title_ar')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" name="title_cku" id="title_cku" class="form-control @error('title_cku') is-invalid @enderror" 
                               value="{{ old('title_cku', $blog->title_cku) }}" placeholder="Enter Blog Title (CKU)">
                        @error('title_cku')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-3">
                      ...
                      <div class="mb-3">
                        ...
                        <textarea name="description_ar" id="description_ar" rows="4" class="form-control @error('description_ar') is-invalid @enderror" 
                                  placeholder="Enter Blog Description">{{ old('description_ar', $blog->description_ar) }}</textarea>
                      ...
                      <div class="mb-3">
                        ...
                        <textarea name="description_cku" id="description_cku" rows="4" class="form-control @error('description_cku') is-invalid @enderror" 
                                  placeholder="Enter Blog Description">{{ old('description_cku', $blog->description_cku) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        @enderror
                        <div>
                            <img id="imagePreview" src="{{ $blog->image }}" alt="Image Preview" 
                                 class="img-fluid" style="max-height: 150px; margin-top: 10px;">
                        </div>
                    </div>
...
<script>
    function previewImage(event) {
        ...
        if (event.target.files[0]) {
            ...
            imagePreview.src = "{{ asset('storage/' . $blog->image) }}";
        }
    }
</script>
