...
<main class="content">
    <div class="container-fluid p-0">
        <h1 class="h3 mb-3">Edit Coupon</h1>
        <div class="card">
            <div class="card-body">
                <form id="editCouponForm" action="{{ route('admin.coupon.update', $coupon->coupon_id) }}" method="POST">
    @csrf
    @method('PATCH') <!-- To specify that this is an update request -->

    <div class="mb-3">
        <label for="coupon_code" class="form-label">Coupon Code</label>
        <div class="input-group">
            <input type="text" name="coupon_code" id="coupon_code" class="form-control @error('coupon_code') is-invalid @enderror" 
                value="{{ old('coupon_code', $coupon->coupon_code) }}" readonly placeholder="Coupon code is readonly">
        </div>
    </div>
   <div class="mb-3">
    <label for="code_format" class="form-label">Code Format</label>
    <select name="code_format" id="code_format" class="form-control @error('code_format') is-invalid @enderror" disabled>
        <option value="">Select Format</option>
        <option value="numeric" {{ old('code_format', $coupon->code_format) === 'numeric' ? 'selected' : '' }}>Numeric</option>
        <option value="alphanumeric" {{ old('code_format', $coupon->code_format) === 'alphanumeric' ? 'selected' : '' }}>Alphanumeric</option>
        <option value="alphabetical" {{ old('code_format', $coupon->code_format) === 'alphabetical' ? 'selected' : '' }}>Alphabetical</option>
    </select>
    <input type="hidden" name="code_format" value="{{ old('code_format', $coupon->code_format) }}">
</div>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="discount_value" id="discount_value" class="form-control @error('discount_value') is-invalid @enderror" 
            value="{{ old('discount_value', $coupon->discount_value) }}" step="0.01" min="0" placeholder="Enter discount value">
    ...
    <div class="mb-3">
        ...
        <input type="number" name="usage_limit_per_coupon" id="usage_limit_per_coupon" class="form-control @error('usage_limit_per_coupon') is-invalid @enderror" 
            value="{{ old('usage_limit_per_coupon', $coupon->usage_limit_per_coupon) }}" min="1" placeholder="Enter usage limit per coupon">
    ...
    <div class="mb-3">
        ...
        <input type="number" name="usage_limit_per_customer" id="usage_limit_per_customer" class="form-control @error('usage_limit_per_customer') is-invalid @enderror" 
            value="{{ old('usage_limit_per_customer', $coupon->usage_limit_per_customer) }}" min="1" placeholder="Enter usage limit per customer">
    ...
    <div class="mb-3">
        ...
        <input type="date" name="start_date" id="start_date" class="form-control @error('start_date') is-invalid @enderror" 
            value="{{ old('start_date', $coupon->start_date) }}" min="{{ now()->toDateString() }}" placeholder="Select start date">
    </div>
    <div class="mb-3">
        <label for="end_date" class="form-label">End Date</label>
        <input type="date" name="end_date" id="end_date" class="form-control @error('end_date') is-invalid @enderror" 
            value="{{ old('end_date', $coupon->end_date) }}" min="{{ now()->toDateString() }}" placeholder="Select end date">
    </div>
    <div class="mb-3">
        ...
        <input type="number" name="min_purchase_amount" id="min_purchase_amount" class="form-control @error('min_purchase_amount') is-invalid @enderror" 
            value="{{ old('min_purchase_amount', $coupon->min_purchase_amount) }}" step="0.01" min="0" placeholder="Enter minimum purchase amount">
    </div>
    <div class="mb-3">
        ...
        <input type="number" name="max_discount_amount" id="max_discount_amount" class="form-control @error('max_discount_amount') is-invalid @enderror" 
            value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}" step="0.01" min="0" placeholder="Enter maximum discount amount">
    </div>
...
$(document).ready(function () {
   ...
   $(document).ready(function () {
        generateCouponCode();  // Auto-generate when the page is ready (optional)
    });
    // Function to generate a random coupon code (optional if you want automatic code generation)
