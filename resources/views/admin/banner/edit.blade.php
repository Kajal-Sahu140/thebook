...
<main class="content">
                ...
                <form id="editBannerForm" action="{{ route('admin.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" 
                            placeholder="Enter Your Title" value="{{ old('title', $banner->title) }}">
                        @error('title')
                        ...
                        <input type="text"name="banner_link"id="banner_link"class="form-control @error('banner_link') is-invalid @enderror"placeholder="Enter your banner link" 
                    ...
                    <div class="mb-3">
                        ...
                        <select name="click_status" id="click_status" class="form-control @error('category_id') is-invalid @enderror">
                            ...
                            <option value="yes" {{ old('click_status', $banner->click_status) == 'yes' ? 'selected' : '' }}>yes</option>
                            <option value="no" {{ old('click_status', $banner->click_status) == 'no' ? 'selected' : '' }}>no</option>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" 
                            rows="4" placeholder="Enter Your Description">{{ old('description', $banner->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @error('web_banner_image')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                        <div id="webPreview" class="media-preview mt-3">
                            @if($banner->web_banner_image)
                                <img src="{{ asset($banner->web_banner_image) }}" alt="Web Banner Image" style="max-height: 150px;">
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @error('app_banner_image')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                        <div id="appPreview" class="media-preview mt-3">
                            @if($banner->app_banner_image)
                                <img src="{{ asset($banner->app_banner_image) }}" alt="App Banner Image" style="max-height: 150px;">
                            @endif
                        </div>
                    </div>
    ...
    <select name="position" id="position" class="form-control @error('position') is-invalid @enderror">
        @if(in_array($banner->id, [8, 9, 10, 11, 12, 13]))  
    ...
    @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="discount" id="discount" class="form-control @error('discount') is-invalid @enderror" 
                            placeholder="Enter Discount" step="1" min="0" value="{{ old('discount', $banner->discount) }}">
                        @error('discount')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="status_inactive" value="de-active" 
                                {{ old('status', $banner->status) === 'de-active' ? 'checked' : '' }}>
...
<div class="mb-3">
    ...
    <input 
        ...
        value="{{ old('start_date', isset($banner->start_date) ? \Carbon\Carbon::parse($banner->start_date)->format('Y-m-d') : '') }}">
    @error('start_date')
        <span class="invalid-feedback">{{ $message }}</span>
    @enderror
</div>
...
<div class="mb-3">
    ...
    <input 
        ...
        value="{{ old('end_date', isset($banner->end_date) ? \Carbon\Carbon::parse($banner->end_date)->format('Y-m-d') : '') }}">
    @error('end_date')
        <span class="invalid-feedback">{{ $message }}</span>
    @enderror
</div>
