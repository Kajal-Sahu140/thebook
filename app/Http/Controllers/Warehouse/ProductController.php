  ...
  public function index(Request $request)
{
    ...
    $query = Product::where('warehouse_id', Auth::guard('warehouse')->user()->id)->orderBy('product_id', 'desc');
    ...
    try {
        ...
        return redirect()->route('warehouse.product')->with('error', 'Failed to retrieve products.');
    }
    return view('warehouse.product.index', compact('products', 'productImages', 'productVariants', 'count'));
}
////////////////////////////////////////////////////////////
public function create()
{
    ...
    $colors=Color::get();
    $sizes=Size::get();
     return view('warehouse.product.create',compact('brands','categories','colors','sizes'));
}
///////////////////////////////////////////////////
    ...
    session()->flash('old_images', $uploadedImages);  // Save image base64 in session for page reload
        ...
        foreach ($validated['variant_colors'] as $index => $colorId) {
            ...
            if (!$price || empty($price)) {
                return redirect()->route('warehouse.product.create')
        ...
        return redirect()->route('warehouse.product')->with('success', 'Product created successfully.');

    } catch (ValidationException $e) {
        \Log::error('Validation error: ' . $e->getMessage());
        return redirect()->route('warehouse.product.create')->withErrors($e->errors())->withInput();
    } catch (\Exception $e) {
        \Log::error('Error creating product: ' . $e->getMessage());
        return redirect()->route('warehouse.product.create')->with('error', 'Failed to create Product.');
    }
}
...
public function destroy($id)
{
    ...
    try {
        ...
        if (!$product) {
            return redirect()->route('warehouse.product')->with('error', 'Product not found.');
        }
        ...
        return redirect()->route('warehouse.product')->with('success', 'Product and its associated data deleted successfully.');
    } catch (\Exception $e) {
        dd($e);
        ...
        return redirect()->route('warehouse.product')->with('error', 'Failed to delete product.');
    }
}
/////////////////////////////////////////////////////////////////
...
public function show($productId)
{
    ...
    return view('warehouse.product.view', compact('product', 'productImages', 'productVariants'));
}
////////////////////
...
function edit($id) {
    ...
    $colors=Color::get();
    $sizes=Size::get();
    return view('warehouse.product.edit', compact('product','brands','categories','colors','sizes'));
}
//////////////////////////////////////////////
  ...
  $decodedId = base64_decode($id);
    try {
       ...
       $validator = Validator::make($request->all(), [
            ...
            'base_price' => 'required|numeric|min:0.01|max:99999999.99', // Limit base price to 10 digits, 2 decimal places
            'discount' => 'required|numeric|min:0|max:100',
       ...
       // dd($validator);
        ...
        if (!$product) {
            return redirect()->route('warehouse.product.index')->with('error', 'Product not found');
        }
        ...
        $variants = [];
foreach ($request->variant_colors as $index => $colorId) {
    // Skip if any required value is missing or blank
    if (
        empty($colorId) || 
        empty($request->variant_sizes[$index]) || 
        empty($request->variant_prices[$index])
    ) {
        continue; // Skip this iteration if any value is blank
    }

    // Add the variant to the array
    $variants[] = [
        'color' => $colorId,
        'size' => $request->variant_sizes[$index],
...
if (!empty($variants)) {
    ...
    return redirect()->route('warehouse.product.edit', $decodedId)
        ->with('error', 'At least one valid variant must be provided.')
        ->withInput();
}

        // Redirect with success message
        return redirect()->route('warehouse.product')->with('success', 'Product updated successfully.');
        ...
        return redirect()->route('warehouse.product.edit', $decodedId)->with('error', 'Failed to update Product.');
    }
}
